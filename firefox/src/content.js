...
(() => {
  ...
  function buildButton(widget) {
    ...
    button.addEventListener('click', async () => {
      ...
      // fetch() and a tainted canvas fail. Hand the URL to the background, which CAN read
      // it once the extension holds host permission for that origin. The content script
      // can't request that permission itself (permissions isn't exposed to content
      // scripts), so the background returns a "grant needed" message and asks for it on the
      // next toolbar-icon click. Same-origin / blob: / data: previews are read locally.
