  ...
  function buildButton(widget) {
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        let resp;
        if (local && local.data) {
          resp = await browser.runtime.sendMessage({ type: 'generate', data: local.data, mediaType: local.mediaType, model, detailed });
        } else if (img && /^https?:/i.test(img.src)) {
          resp = await browser.runtime.sendMessage({ type: 'generate', imageUrl: img.src, model, detailed });
        } else {
