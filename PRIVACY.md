# Privacy Policy — AI Alt Text for Mastodon
...
- **Your settings** (selected model, editable pricing table, list of Mastodon
  instance domains, and a running session-cost total) are also stored only in
...
- **scripting** — to add the alt-text button to the Mastodon instances you
  explicitly approve.
- **Host access to your Mastodon instance(s)** — requested per-instance, only
  when you add a domain on the options page, so the button can appear in that
  site's composer.
