...
// request is invoked synchronously in the gesture; a rare service-worker restart just falls
// back to opening options, and the user re-triggers by clicking Generate again.
let pendingGrantOrigin = null;

// Fetch a cross-origin image URL and return { data (base64), mediaType }. Runs in the
// service worker, which — unlike the page/content script — can read a cross-origin
// response once the extension holds host permission for its origin (the content script
// requests that permission before sending the URL). Content-Type drives media_type,
// falling back to the file extension so a webp isn't mislabeled. Chunked base64 avoids
// blowing the call stack on a large image.
